...
public class SearchFilterUtil {
	...
	public enum OperatorUtil{
        EQ, LIKE, GT, LT, GTE, LTE,NEQ,IN,NULL,NOTNULL
	}
	...
	private static List<String> filterNames = Lists.newArrayList("class","version","userImgUrl","countFlag");
	...
	public static void copyBeansNull(Object target, Object original) throws Exception {
		Class type = original.getClass();
		BeanInfo beanInfo = Introspector.getBeanInfo(type);
		...
		for (int i = 0; i< propertyDescriptors.length; i++) {
			...
			if (!propertyName.equals("class")) {
				...
				if(readMethod!=null){
                    ...
                    if(result != null && result instanceof Map && ((Collection) result).isEmpty()){
                        continue;
                    }
                    Method writeMethod = descriptor.getWriteMethod();
                    if( writeMethod != null){
